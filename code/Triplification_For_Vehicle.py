...
for _,row in vehicle_data.iterrows():
    ...
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasVehicleMake'],row['MAKENAME'],pfs["xsd"].string)
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasBodyType'],row['BODY_TYPNAME'],pfs["xsd"].string)
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasVehicleModel'],row['MAK_MODNAME'],pfs["xsd"].string)
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasVehicleManufacturingYear'],row['MOD_YEARNAME'],pfs["xsd"].string)
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasWeightRange'],row['GVWR_TONAME'],pfs["xsd"].string)
    add_literal_property(graph, vehicle_uri, pfs['rc-ont']['hasVIN'],row['VIN'],pfs["xsd"].string)
    add_subclass_property(graph, vehicle_uri, participant_uri)

    #vehicle in accident_uri
    vehicle_in_accident_uri = URIRef(pfs['rc-res'][f"VehicleInCrash_{crash_id}_{row['VEH_NO']}"])
    graph.add((vehicle_in_accident_uri, a, pfs["rc-ont"]["VehicleInCrash"]))
    ...
    add_literal_property(graph, vehicle_in_accident_uri, pfs['rc-ont']['hasTrailingUnit'],row['TOW_VEHNAME'],pfs["xsd"].string)
    ...
    add_object_property(graph,vehicle_uri,pfs['rc-ont']['performsVehicleInCrash'],vehicle_in_accident_uri)
    add_object_property(graph,crash_uri,pfs['rc-ont']['providesVehicleInCrash'],vehicle_in_accident_uri)
    ...
    recent_driving_conviction_uri = URIRef(pfs['rc-res'][f"RecentDrivingConviction_{crash_id}_{row['VEH_NO']}_1"]) #_1 is added as all driver has index of 1 in every vehicle
    ...
    add_subclass_property(graph,road_condition_uri,condition_uri)
